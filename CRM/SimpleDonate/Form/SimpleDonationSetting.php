...
class CRM_SimpleDonate_Form_SimpleDonationSetting extends CRM_Admin_Form_Setting {
  ...
  public function transactDonation() {
    ...
    //retreive addition value depending on payment processor selected 
    $paymentParams = CRM_SimpleDonate_Form_PaymentProccessor::getadditionalPaymentRes($params, $isTest);
    ...
    $donation = self::createSimpleContribution($contactID, $params, $isTest, $creditInfo, $paymentParams);
    echo json_encode($donation);
    CRM_Utils_System::civiExit();
  }
  
  public function createSimpleContribution($contactID, $params, $isTest, $creditInfo, $paymentParams=NULL) {
    ...
    if (!empty($paymentParams)) {
      $contributionparams = array_merge($contributionparams,$paymentParams);
   }
